...
macro_rules!objc_block {
    (move || $ (: $ return_ty: ty) ? $ body: block) => {{
        ...
        struct BlockLiteral {
            ...
            invoke: extern "C" fn(*mut BlockLiteral) $ ( -> $ return_ty) ?,
            descriptor: *const BlockDescriptor,
            inner: ::std::sync::Arc<::std::sync::Mutex<dyn Fn() $ ( -> $ return_ty) ? >>,
        }
        ...
        #[allow(unused_unsafe)]
        extern "C" fn invoke(literal: *mut BlockLiteral) $ ( -> $ return_ty) ? {
            let literal = unsafe { &mut *literal };
            literal.inner.lock().unwrap()()
        }
        ...
        BlockLiteral {
            ...
            inner: ::std::sync::Arc::new(::std::sync::Mutex::new(move || { $ body })),
        }
    }};
