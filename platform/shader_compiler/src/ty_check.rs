...
use {
    crate::{
        lhs_check::LhsChecker, makepad_live_compiler::*, shader_ast::*,
        shader_registry::ShaderRegistry, swizzle::Swizzle, util::CommaSep,
    },
    std::{cell::Cell, collections::BTreeSet, fmt::Write, rc::Rc},
};
...
impl<'a> TyChecker<'a> {
    ...
    fn ty_check_closure_def(&mut self, index: ClosureDefIndex) -> Result<Ty, LiveError> {
    ...
    */
                    ...
                    ScopeSymKind::Closure {
                        ...
                        if closure_args.len() > 0 {
                            return Err(LiveError {
                                ...
                                span: span.into(),
                                message: format!(
                                    "Cannot pass closures to closures, please implement"
                                ),
                            });
                        }
                        outer_param_index.set(Some(*param_index));
        ...
        match self.check_params_against_args(span, &fn_def.params, arg_exprs) {
            Err(err) => Err(LiveError {
                ...
                message: format!(
                    "function: `{}`: {}",
                    self.shader_registry
                        .fn_ident_from_ptr(self.live_registry, fn_ptr),
                    err.message
                ),
            }),
            Ok(closure_args) => {
    ...
    fn ty_check_field_expr(
        ...
        match ty {
            ...
            Ty::Struct(struct_ptr) => Ok(self
                ...
                .unwrap()
                .clone()),
            Ty::DrawShader(shader_ptr) => Ok(self
                .shader_registry
                .draw_shader_defs
                .get(&shader_ptr)
                .unwrap()
                ...
                .ok_or(LiveError {
                    ...
                    span: span.into(),
                    message: format!(
                        "field `{}` is not defined on shader `{:?}`",
                        field_ident, shader_ptr
                    ),
                })?
                .ty_expr
                .ty
                ...
                if actual_slots > expected_slots {
                    return Err(LiveError {
                        ...
                        message: format!(
                            ...
                            ty_lit, expected_slots, actual_slots,
                        )
                        .into(),
                    });
                }
                ...
                Some(scopesym) => {
                    ...
                    match &scopesym.kind {
                        ScopeSymKind::MutLocal => {
                            kind.set(Some(VarKind::MutLocal {
                                ident,
                                shadow: scopesym.sym.shadow,
                            }));
                            return Ok(scopesym.sym.ty.clone());
                        }
                        ScopeSymKind::Local => {
                            kind.set(Some(VarKind::Local {
                                ident,
                                shadow: scopesym.sym.shadow,
                            }));
                            return Ok(scopesym.sym.ty.clone());
                        ...
                        ScopeSymKind::Closure { .. } => {
                            ...
                            return Err(LiveError {
                                ...
                                span: span.into(),
                                message: format!(
                                    "`{}` is a closure and cannot be used as a variable",
                                    ident
                                ),
                            });
                        }
        ...
        match var_resolve {
            ...
            VarResolve::Function(fn_ptr) => {
                return Err(LiveError {
                    ...
                    span: span.into(),
                    message: format!(
                        "`{}` implement using functions as closure args",
                        ident.unwrap()
                    ),
                })
            }
    ...
    fn ty_check_struct_cons(
                ...
                .find(|field| field.ident == *ident)
            {
                ...
                if field_ty.as_ref() != my_ty.as_ref() {
                    return Err(LiveError {
                        ...
                        span: span.into(),
                        message: format!(
                            "field `{}` is the wrong type {} instead of {}",
                            ident,
                            my_ty.as_ref().unwrap(),
                            field_ty.as_ref().unwrap()
                        ),
                    });
                }
            } else {
        ...
        for field in &struct_decl.fields {
            if args
                .iter()
                .position(|(ident, expr)| ident == &field.ident)
                .is_none()
            {
                return Err(LiveError {
                    origin: live_error_origin!(),
