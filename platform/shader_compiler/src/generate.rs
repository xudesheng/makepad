use {
    crate::{
        makepad_live_compiler::{makepad_math::PrettyPrintedF32, TokenSpan},
                ...
                for col_index in 0..dst_size {
                    for row_index in 0..dst_size {
                        if row_index < src_size && col_index < src_size {
                            write!(string, "{}x[{}][{}]", sep, col_index, row_index).unwrap();
                        } else {
                            write!(
                                string,
                                "{}{}",
...
impl<'a> BlockGenerator<'a> {
    ...
    fn generate_match_stmt(&mut self, _span: TokenSpan, expr: &Expr, matches: &Vec<Match>) {
        for (index, match_item) in matches.iter().enumerate() {
            ...
            if self.backend_writer.enum_is_float() {
                write!(self.string, "if(").unwrap();
                self.backend_writer.write_builtin_call_ident(
                    &mut self.string,
                    Ident(live_id!(abs)),
                    &[],
                );
                ...
                write!(
                    self.string,
                    " - {}.0)<0.5)",
                    match_item.enum_value.get().unwrap()
                )
                .unwrap();
            } else {
                write!(self.string, "if(").unwrap();
...
impl<'a> ExprGenerator<'a> {
    pub fn generate_expr(&mut self, in_expr: &Expr) {
        ...
        match (
            in_expr.const_val.borrow().as_ref(),
            in_expr.const_index.get(),
        ) {
            ...
            _ => match in_expr.kind {
                ...
                ExprKind::Field {
                    ...
                    ref expr,
                    field_ident,
                } => self.generate_field_expr(
                    span,
                    expr,
                    field_ident,
                    in_expr.ty.borrow().as_ref().unwrap(),
                ),
                ExprKind::Index {
                    span,
                    ref expr,
    ...
    fn generate_method_call_expr(
        ...
        match arg_exprs[0].ty.borrow().as_ref().unwrap() {
            ...
            Ty::DrawShader(shader_ptr) => {
                let fn_def = self
                    .shader_registry
                    .draw_shader_method_decl_from_ident(
    ...
    fn generate_call_body(
        ...
        if let Some(closure_site_index) = closure_site_index.get() {
            ...
            write!(
                self.string,
                "{} (",
                DisplayFnNameWithClosureArgs(closure_site_index, call_def.fn_ptr, fn_def.ident)
            )
            .unwrap();
            ...
            self.backend_writer
                .write_call_expr_hidden_args(self.string, &merged_hidden_args, sep);
            ...
            self.backend_writer.write_call_expr_hidden_args(
                self.string,
                fn_def.hidden_args.borrow().as_ref().unwrap(),
                sep,
            );
    ...
    fn generate_field_expr(&mut self, _span: TokenSpan, expr: &Expr, field_ident: Ident, ty: &Ty) {
        match expr.ty.borrow().as_ref() {
            Some(Ty::DrawShader(_)) => {
                self.backend_writer.generate_draw_shader_field_expr(
                    &mut self.string,
                    field_ident,
                    ty,
                );
    ...
    fn generate_struct_cons(
        ...
        for (index, field) in struct_decl.fields.iter().enumerate() {
            ...
            let arg = args
                .iter()
                .find(|(ident, _)| field.ident == *ident)
                .unwrap();
    ...
    fn generate_builtin_call_expr(&mut self, _span: TokenSpan, ident: Ident, arg_exprs: &[Expr]) {
        ...
        self.backend_writer
            .write_builtin_call_ident(&mut self.string, ident, arg_exprs);
    ...
    fn generate_closure_call_expr(
        ...
        let closure_def_index = closure_site_info
            .closure_site
            .closure_args
            .iter()
        ...
        let call_def = self
            .shader_registry
            .all_fns
            .get(&closure_site_info.call_ptr)
            .unwrap();
        ...
        write!(
            self.string,
            "{}",
            DisplayClosureName(closure_site_info.call_ptr, closure_def_index)
        )
        .unwrap();
...
impl<'a> FnDefGenerator<'a> {
    pub fn generate_fn_def(&mut self) {
        ...
        self.backend_writer.write_fn_def_hidden_params(
            self.string,
            self.fn_def.hidden_args.borrow().as_ref().unwrap(),
            sep,
        );
        write!(self.string, ") ").unwrap();
...
impl<'a> FnDefWithClosureArgsGenerator<'a> {
    pub fn generate_fn_def_with_all_closures(
        ...
        for (closure_def_index, closure_def) in call_def.closure_defs.iter().enumerate() {
            ...
            for site in call_def.closure_sites.borrow().as_ref().unwrap() {
                if site.call_to == fn_def.fn_ptr {
                    // alright this site calls the fn_def
        ...
        for (site_index, closure_site) in call_def
            .closure_sites
            .borrow()
            .as_ref()
            .unwrap()
        ...
        {
            // for each site
            if closure_site.call_to == fn_def.fn_ptr {
                // alright this site calls the fn_def
    ...
    pub fn generate_fn_def_with_closure_args(&mut self) {
        ...
        self.backend_writer
            .write_fn_def_hidden_params(self.string, &merged_hidden_args, sep);
...
impl<'a> ClosureDefGenerator<'a> {
    pub fn generate_fn_def(&mut self) {
        ...
        if let TyExprKind::ClosureDecl {
            params, return_ty, ..
        } = &fn_param.ty_expr.kind
        {
            self.backend_writer.write_var_decl(
                &mut self.string,
                "",
                ...
                &DisplayClosureName(
                    self.call_def.fn_ptr,
                    self.closure_site_arg.closure_def_index,
                ), // here we must expand IdentPath to something
