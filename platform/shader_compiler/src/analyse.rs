use {
    crate::{
        ...
        makepad_live_compiler::{
            ...
            LiveError, LiveErrorOrigin, LiveRegistry, TokenSpan,
        },
        shader_ast::*,
                ...
                match ty {
                    ...
                    _ => {
                        return Err(LiveError {
                            ...
                            span: decl.span.into(),
                            message: String::from(
                                "texture must be either a texture2D or a textureOES (android)",
                            ),
                        })
                    }
                }
...
impl<'a> ConstAnalyser<'a> {
    ...
    pub fn analyse_const_decl(&mut self) -> Result<(), LiveError> {
        let expected_ty = self
            .ty_checker()
            .ty_check_ty_expr(&self.const_def.ty_expr)?;
        ...
        self.const_evaluator()
            .const_eval_expr(&self.const_def.expr)?;
        Ok(())
    }
}
...
impl<'a> FnDefAnalyser<'a> {
            ...
            if let Ty::Void = ty_expr.ty.borrow().as_ref().unwrap() {
                ...
                if !self.fn_def.has_return.get() {
                    return Err(LiveError {
                        ...
                        message: format!("Function has no return",),
                    });
                }
            }
        }
    ...
    fn analyse_closures(&mut self) -> Result<(), LiveError> {
        ...
        for closure_site in &mut closure_sites {
            let fn_decl = self
                .shader_registry
                .all_fns
                .get(&closure_site.call_to)
                .unwrap();

            // lets start the closure
            for closure_arg in &closure_site.closure_args {
                let mut scopes = closure_scopes
                    .get_mut(&closure_arg.closure_def_index)
                    .unwrap();
    ...
    fn analyse_for_stmt(
        ...
        self.scopes.push_scope();
        self.scopes
            .insert_sym(span, ident, Ty::Int, ScopeSymKind::Local);
            ...
            for match_item in matches {
                ...
                if match_item.enum_name.0 != shader_enum.enum_name {
                    return Err(LiveError {
                        ...
                        span: span.into(),
                        message: format!(
                            "Enum name mismatched, expected {} got {}",
                            shader_enum.enum_name, match_item.enum_name.0
                        ),
                    }
                    .into());
                }

                if let Some(pos) = shader_enum
                    .variants
                    .iter()
                    .position(|id| *id == match_item.enum_variant.0)
                {
                    ...
                    return Err(LiveError {
                        ...
                        message: format!(
                            ...
                            match_item.enum_name.0, match_item.enum_variant.0
                        ),
                    }
                    .into());
                }
