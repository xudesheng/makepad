use {
    crate::{
        live_ptr::LiveFileId,
        makepad_live_tokenizer::{Delim, FullToken, LiveId},
        makepad_math::Vec4,
        span::TextSpan,
    },
    std::{fmt, ops::Deref, ops::DerefMut, sync::Arc},
};
...
impl LiveToken {
    ...
    pub fn is_value_type(&self) -> bool {
        matches!(
            self,
            LiveToken::Color(_) | LiveToken::Bool(_) | LiveToken::Int(_) | LiveToken::Float(_)
        )
    }
    ...
    pub fn is_parse_equal(&self, other: &LiveToken) -> bool {
        match self {
            LiveToken::String(p) => {
                if let LiveToken::String(o) = other {
                    *p == *o
                } else {
                    false
                }
            ...
            LiveToken::Punct(p) => {
                if let LiveToken::Punct(o) = other {
                    *p == *o
                } else {
                    false
                }
            ...
            LiveToken::Ident(p) => {
                if let LiveToken::Ident(o) = other {
                    *p == *o
                } else {
                    false
                }
            ...
            LiveToken::Close(p) => {
                if let LiveToken::Close(o) = other {
                    *p == *o
                } else {
                    false
                }
            ...
            LiveToken::Int(_) => {
                if let LiveToken::Int(_) = other {
                    true
                } else {
                    matches!(other, LiveToken::Float(_))
                }
            }
            LiveToken::Float(_) => {
                if let LiveToken::Float(_) = other {
                    true
                } else {
                    matches!(other, LiveToken::Int(_))
                }
            }
...
impl LiveTokenId {
    pub fn new(file_id: LiveFileId, token: usize) -> Self {
        ...
        LiveTokenId((((file_id as u32 + 1) & 0x3ff) << 18) | ((token as u32) & 0x3ffff))
    }
...
impl fmt::Debug for LiveTokenId {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "TokenId(token_index:{}, file_id:{})",
            self.token_index(),
            self.file_id().unwrap_or(LiveFileId::new(0)).to_index()
        )
    }
}
