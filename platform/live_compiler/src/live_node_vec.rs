...
use {
    crate::{
        live_node::{InlineString, LiveNode, LiveNodeOrigin, LiveProp, LivePropType, LiveValue},
        live_token::LiveTokenId,
        //makepad_error_log::*,
        makepad_derive_live::live_object,
        makepad_live_tokenizer::LiveId,
        makepad_math::{Vec2, Vec3, Vec4},
    },
    std::{fmt::Write, iter, sync::Arc},
};
...
pub trait LiveNodeVecApi {
    ...
    fn replace_or_insert_last_node_by_path(
        &mut self,
        start_index: usize,
        path: &[LiveProp],
        other: &[LiveNode],
    ...
    T: AsRef<[LiveNode]>,
{
    ...
    fn scope_up_by_name(&self, index: usize, name: LiveProp) -> Option<usize> {
        ...
        loop {
            ...
            if stack_depth == 0
                && self_ref[index].id == name.0
                && self_ref[index].origin.has_prop_type(name.1)
                && !self_ref[index].is_close()
            {
                // valuenode
                return Some(index);
            }
    ...
    fn child_or_append_index_by_name(
        ...
        while index < self_ref.len() {
            if self_ref[index].is_open() {
                if stack_depth == 1 {
                    if self_ref[index].origin.has_prop_type(child_name.1)
                        && self_ref[index].id == child_name.0
                    {
                        return Ok(index);
                    }
                }
    ...
    fn to_string(&self, parent_index: usize, max_depth: usize) -> String {
        ...
        while index < self_ref.len() {
            ...
            match &node.value {
                ...
                LiveValue::Expr => {
                    writeln!(f, "{} <Expr> {:?}", node.id, pt).unwrap();
                    stack_depth += 1;
                }
                ...
                LiveValue::DSL {
                    ...
                    writeln!(
                        f,
                        "<DSL> {} :token_start:{}, token_count:{} expand_index:{:?}",
                        node.id, token_start, token_count, expand_index
                    )
                    .unwrap();
                }
                LiveValue::Import(live_import) => {
                    writeln!(
                        f,
                        "<Import> {}::{} as {}",
                        live_import.module_id, live_import.import_id, node.id
                    )
                    .unwrap();
                }
                LiveValue::Font(s) => {
...
impl LiveNodeVecApi for LiveNodeVec {
    fn insert_children_from_other(
        ...
        if !other[source_index].is_open() {
            panic!(
                "Failed to insert non-open LiveNode: {:?}, \
                \n\t --> Double-check your usage of {:?}!",
                &other[source_index], &other[source_index].id,
            );
        }
        ...
        self.splice(
            ...
            other[source_index + 1..(source_index + 1 + num_children)]
                .iter()
                .cloned(),
        );
    }
    ...
    fn insert_children_from_self(&mut self, source_index: usize, insert_point: usize) {
        ...
        self.splice(
            insert_point..insert_point,
            iter::repeat(LiveNode::empty()).take(num_children),
        );

        let source_index = if insert_point < source_index {
            source_index + num_children
        } else {
            source_index
        };
    ...
    fn insert_node_from_self(&mut self, source_index: usize, insert_point: usize) -> usize {
        ...
        self.splice(
            insert_point..insert_point,
            iter::repeat(LiveNode::empty()).take(num_nodes),
        );
    ...
    fn insert_node_from_other(
        ...
        self.splice(
            insert_point..insert_point,
            other[source_index..(source_index + num_nodes)]
                .iter()
                .cloned(),
        );
    ...
    fn write_field_nodes(&mut self, path: &[LiveId], nodes: &[LiveNode]) {
        ...
        self.replace_or_insert_last_node_by_path(0, &ids[0..path.len()], nodes);
        if was_empty {
            self.close();
        }
    }
    ...
    fn push_string(&mut self, id: LiveId, v: &str) {
        ...
        if let Some(inline_str) = InlineString::from_str(v) {
            self.push(LiveNode {
                origin: LiveNodeOrigin::empty(),
                id,
                value: LiveValue::InlineString(inline_str),
            });
        } else {
            self.push(LiveNode {
                origin: LiveNodeOrigin::empty(),
                id,
                value: LiveValue::String(Arc::new(v.to_string())),
            });
        }
    }
    ...
    fn root2(&mut self) {
        self.push(LiveNode {
            origin: LiveNodeOrigin::empty(),
            id: LiveId(0),
            value: LiveValue::Root(Default::default()),
        })
    }
}
