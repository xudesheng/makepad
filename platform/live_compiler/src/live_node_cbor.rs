use {
    crate::{live_node::*, makepad_live_tokenizer::LiveId},
    std::sync::Arc,
};
        ...
        while index < nodes.len() {
            ...
            if node.value.is_close() {
                ...
                if item.count > std::u16::MAX as usize {
                    out[item.index] = if item.has_keys {
                        CBOR_MAP_32
                    } else {
                        CBOR_ARRAY_32
                    };
            ...
            match &node.value {
                ...
                LiveValue::Close => {}
                // TODO ITEMS
                LiveValue::Id(_) => return Err("Cannot serialise LiveValue::Id".into()),
                LiveValue::Clone { .. } => return Err("Cannot serialise LiveValue::Clone".into()), // subnodes including this one
                LiveValue::ExprBinOp(_) => {
                    return Err("Cannot serialise LiveValue::ExprBinOp".into())
                }
                LiveValue::ExprUnOp(_) => return Err("Cannot serialise LiveValue::ExprUnOp".into()),
                LiveValue::ExprMember(_) => {
                    return Err("Cannot serialise LiveValue::ExprMember".into())
                }
                LiveValue::Expr { .. } => return Err("Cannot serialise LiveValue::Expr".into()),
                LiveValue::ExprCall { .. } => {
                ...
                LiveValue::Class { .. } => return Err("Cannot serialise LiveValue::Class".into()), // subnodes including this one
                LiveValue::Deref { .. } => return Err("Cannot serialise LiveValue::Deref".into()), // subnodes including this one
                LiveValue::DSL { .. } => return Err("Cannot serialise LiveValue::DSL".into()),
                LiveValue::Import(..) => return Err("Cannot serialise LiveValue::Import".into()),
                LiveValue::Root { .. } => return Err("Cannot serialise LiveValue::Registry".into()),
                LiveValue::IdPath(..) => return Err("Cannot serialise LiveValue::IdPath".into()),
                LiveValue::Font { .. } => return Err("Cannot serialise LiveValue::Font".into()),
            }
            index += 1;
        }
        if stack.len() > 1 {
...
impl LiveNodeVecFromCbor for Vec<LiveNode> {
    fn from_cbor(&mut self, data: &[u8]) -> Result<(), LiveNodeFromCborError> {
        ...
        fn assert_len(o: usize, len: usize, data: &[u8]) -> Result<(), LiveNodeFromCborError> {
            if o + len > data.len() {
                panic!()
            } //return Err(LiveNodeFromBinaryError::OutOfBounds);}
            Ok(())
        }
        ...
        fn decode_str<'a>(
            data: &'a [u8],
            o: &mut usize,
        ) -> Result<Option<&'a str>, LiveNodeFromCborError> {
            assert_len(*o, 1, data)?;
        ...
        fn decode_array_len(
            data: &[u8],
            o: &mut usize,
        ) -> Result<Option<usize>, LiveNodeFromCborError> {
            assert_len(*o, 1, data)?;
        ...
        while o < data.len() {
            ...
            let id = if stack_item.has_keys {
                ...
                if id.is_none() {
                    return Err(LiveNodeFromCborError::ExpectedId);
                }
                id.unwrap()
            } else {
                LiveId(0)
            ...
            if let Some(v) = decode_i64(data, &mut o)? {
                ...
                match data[o] {
                    ...
                    _ => return Err(LiveNodeFromCborError::UnexpectedValue),
                }
            };
        }
        if stack.last().unwrap().count == stack.last().unwrap().len {
