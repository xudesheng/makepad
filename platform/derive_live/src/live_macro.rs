...
fn parse_object(parser: &mut TokenParser, tb: &mut TokenBuilder) -> Result<(), TokenStream> {
    while !parser.eat_eot() {
        let live_id = if parser.is_bracket() {
            // computed id
            parser.open_group();
            parser.eat_level()
        } else {
...
fn parse_value(
    node_start: TokenStream,
    parser: &mut TokenParser,
    tb: &mut TokenBuilder,
) -> Result<(), TokenStream> {
    if parser.is_paren() {
        // its a Rust injection
        parser.open_group();
        let arg = parser.eat_level();
        tb.add("LiveNode{")
            .stream(Some(node_start))
            .add(",value:(")
            .stream(Some(arg))
            .add(").to_live_value()},");
        ...
        // its an array
        tb.add("LiveNode{")
            .stream(Some(node_start.clone()))
            .add(",value:LiveValue::Array},");
        parser.open_group();
        parse_array(parser, tb)?;
        tb.add("LiveNode{")
            .stream(Some(node_start))
            .add(",value:LiveValue::Close},");
    } else if parser.is_brace() {
        // its a bare class
        tb.add("LiveNode{")
            .stream(Some(node_start.clone()))
            .add(",value:LiveValue::Object},");
        parser.open_group();
        parse_object(parser, tb)?;
        tb.add("LiveNode{")
            .stream(Some(node_start))
    ...
    // key values
        ...
        if parser.is_brace() {
            ...
            tb.add("LiveNode{")
                .stream(Some(node_start))
                .add(",value:LiveValue::Bool(true)},");
        } else if class == "false" {
            tb.add("LiveNode{")
                .stream(Some(node_start))
                .add(",value:LiveValue::Bool(false)},");
        } else {
            if class.chars().next().unwrap().is_uppercase() {
                tb.add("LiveNode{")
                    .stream(Some(node_start))
                    .add(",value:LiveValue::BareEnum(");
                tb.add("LiveId(").suf_u64(class_id).add("))},");
            } else {
                tb.add("LiveNode{")
                    .stream(Some(node_start))
                    .add(",value:LiveValue::Id(");
                tb.add("LiveId(").suf_u64(class_id).add("))},");
            }
        }
    } else if parser.eat_punct_alone('#') {
        ...
        if bytes[0] == b'"' {
            ...
            tb.add("LiveNode{")
                .stream(Some(node_start))
                .add(",value:LiveValue::Str(")
                .string(val)
                .add(")},");
        } else if s == "true" || s == "false" {
            tb.add("LiveNode{")
                .stream(Some(node_start))
                .add(",value:LiveValue::Bool(")
                .ident(&s)
                .add(")},");
