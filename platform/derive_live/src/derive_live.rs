...
use makepad_micro_proc_macro::{
    error_result, unwrap_option, Attribute, StructField, TokenBuilder, TokenParser,
};
...
fn derive_live_impl_inner(
    ...
    if parser.eat_ident("struct") {
        ...
        let animator_field = fields
            .iter()
            .find(|field| field.attrs.iter().any(|a| a.name == "animator"));
        /*
        ...
        */
        if let Some(animator_field) = animator_field {
            ...
            tb.add(
                "                            apply.override_from(ApplyFrom::AnimatorInit,|apply|{",
            );
            ...
            tb.add("                if let Some(live_ptr) = self.")
                .ident(&animator_field.name)
                .add(".live_ptr {");
            ...
            tb.add("                    if !self.")
                .ident(&animator_field.name)
                .add(".animator_in_state(cx, state_pair){");
            tb.add("                       self.")
                .ident(&animator_field.name)
                .add(".animate_to_live(cx, state_pair);");
            tb.add("                    }");
            tb.add("                    index = nodes.skip_node(index);");
            tb.add("                }");
        ...
        let layout_fields = [
            "scroll", "clip_x", "clip_y", "padding", "align", "flow", "spacing",
        ];
        ...
        if let Some(deref_field) = deref_field {
            tb.add("    self.")
                .ident(&deref_field.name)
                .add(".apply_reset(cx, apply, start_index, nodes);");
        }

        for field in &fields {
            if field.attrs.iter().any(|a| a.name == "live") {
                tb.add("let mut ")
                    .ident(&format!("has_{}", field.name))
                    .add(" = false;");
        ...
        for field in &fields {
            if field.attrs.iter().any(|a| a.name == "live") {
                tb.add("            LiveId(")
                    .suf_u64(LiveId::from_str(&field.name).0)
                    .add(")=>")
                    .ident(&format!("has_{}", field.name))
                    .add("= true,");
            } else if field.attrs.iter().any(|a| a.name == "walk") {
        ...
        for field in &fields {
            if field.attrs.iter().any(|a| a.name == "live")
                || field.attrs.iter().any(|a| a.name == "animator")
            {
                tb.add("        LiveId(")
                    .suf_u64(LiveId::from_str(&field.name).0)
                    .add(")=>self.")
                    .ident(&field.name)
                    .add(".apply(cx, apply, index, nodes),");
            } else if field.attrs.iter().any(|a| a.name == "walk") {
                for field in &fields {
                    for f in walk_fields {
                        if f == field.name {
                            return error_result(&format!(
                                "Name collision between walk splat and {}",
                                field.name
                            ));
                        }
                    }
                ...
                for field in &fields {
                    for f in layout_fields {
                        if f == field.name {
                            return error_result(&format!(
                                "Name collision between layout splat and {}",
                                field.name
                            ));
                        }
                    }
        ...
        if let Some(deref_field) = deref_field {
            tb.add("            _=> self.")
                .ident(&deref_field.name)
                .add(".apply_value(cx, apply, index, nodes)");
        } else {
        ...
        if let Some(deref_field) = deref_field {
            ...
            tb.add(
                "        <Self as LiveHook>::apply_value_instance(self, cx, apply, index, nodes)",
            );
        }
        tb.add("        }");
        tb.add("    }");
        ...
        if let Some(deref_field) = deref_field {
            tb.add("    self.")
                .ident(&deref_field.name)
                .add(".deref_before_apply(cx, apply, index, nodes);");
        }
        tb.add("    }");
        ...
        if let Some(deref_field) = deref_field {
            tb.add("    self.")
                .ident(&deref_field.name)
                .add(".deref_after_apply(cx, apply, index, nodes);");
        }
        ...
        tb.add("            let struct_id = LiveId(")
            .suf_u64(LiveId::from_str(&struct_name).0)
            .add(");");
        tb.add("            if !nodes[start_index].value.is_structy_type(){");
        ...
        if let Some(animator_field) = animator_field {
            // apply the default states
            tb.add("            if nodes[index].id == live_id!(")
                .ident(&animator_field.name)
                .add("){animator_index = Some(index);}");
        }
        ...
        for field in &fields {
            if let Some(attr) = field.attrs.iter().find(|a| {
                a.name == "animator" || a.name == "live" || a.name == "calc" || a.name == "deref"
            }) {
                tb.add("fields.push(LiveTypeField{id:LiveId::from_str_with_lut(")
                    .string(&field.name)
                    .add(").unwrap(),");
                ...
                match unwrap_option(field.ty.clone()) {
                    Ok(inside) => {
                        ...
                        tb.add("live_type_info:")
                            .add("<")
                            .stream(Some(inside))
                            .add("as LiveNew>::live_type_info(cx),");
                        tb.add("live_field_kind: LiveFieldKind::LiveOption");
                    }
                    Err(not_option) => {
                        tb.add("live_type_info:")
                            .add("<")
                            .stream(Some(not_option))
                            .add("as LiveNew>::live_type_info(cx),");
        ...
        tb.add("            live_ignore: ")
            .ident(if live_ignore { "true" } else { "false" })
            .add(",");
        tb.add("            fields,");

        tb.add("            type_name: LiveId::from_str_with_lut(")
            .string(&struct_name)
            .add(").unwrap()");
        tb.add("        }");
        tb.add("    }");
        ...
        for field in &fields {
            if field
                .attrs
                .iter()
                .any(|a| a.name == "live" || a.name == "calc" || a.name == "deref")
            {
                match unwrap_option(field.ty.clone()) {
                    Ok(inside) => {
                        tb.add("<")
                            .stream(Some(inside))
                            .add("as LiveNew>::live_design_with(cx);");
                    }
                    Err(not_option) => {
                        tb.add("<")
                            .stream(Some(not_option))
        ...
        let is_u32_enum = main_attribs.iter().any(|attr| {
            attr.name == "repr" && attr.args.as_ref().unwrap().to_string().to_lowercase() == "u32"
        });
        if is_u32_enum {
            ...
            for item in &items {
                match item.kind {
                    EnumKind::Bare => {
                        tb.add("variants.push(LiveId::from_str_with_lut(")
                            .string(&item.name)
                            .add(").unwrap());");
                    }
                    EnumKind::Named(_) | EnumKind::Tuple(_) => {
                        return error_result(
                            "For repr(u32) shader-accessible enums only bare values are supported",
                        );
                    }
                }
            }
            tb.add("        cx.shader_registry.register_enum(LiveType::of::<")
                .ident(&enum_name)
                .add(">(),ShaderEnum{enum_name:LiveId::from_str_with_lut(")
                .string(&enum_name)
                .add(").unwrap(),variants});");
        }
        ...
        tb.add("        let enum_id = LiveId(")
            .suf_u64(LiveId::from_str(&enum_name).0)
            .add(");");
        ...
        for item in &items {
            if let EnumKind::Bare = item.kind {
                tb.add("            LiveId(")
                    .suf_u64(LiveId::from_str(&item.name).0)
                    .add(")=>{index += 1;*self = Self::")
                    .ident(&item.name)
                    .add("},");
            }
        }
        ...
        for item in &items {
            if let EnumKind::Named(fields) = &item.kind {
                ...
                for field in fields {
                    tb.ident(&field.name)
                        .add(":")
                        .ident(&format!("prefix_{}", field.name))
                        .add(",");
                }
                tb.add("                } = self {");
                ...
                for field in fields {
                    tb.add("                        LiveId(")
                        .suf_u64(LiveId::from_str(&field.name).0)
                        .add(")=>{index = (*");
                    tb.ident(&format!("prefix_{}", field.name))
                        .add(").apply(cx, apply, index, nodes);},");
                }
                tb.add("                            _=>{");
