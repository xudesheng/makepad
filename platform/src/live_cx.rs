use crate::{
    ...
    makepad_live_compiler::{
        LiveError, LiveFileChange, /*LiveTokenId,*/
        LiveFileId, LiveId, LiveModuleId, LiveNode, LiveNodeSliceApi, LiveProp,
        /*LiveToken,*/
        LivePtr, LiveValue, TextPos,
    },
    makepad_live_tokenizer::{live_error_origin, LiveErrorOrigin},
    makepad_micro_serde::*,
    studio::{StudioToApp, StudioToAppVec},
...
impl Cx {
    pub fn apply_error_tuple_enum_arg_not_found(
        &mut self,
        ...
        self.apply_error(
            ...
            nodes,
            format!(
                "tuple enum too many args for {}::{} arg no {}",
                enum_id, base, arg
            ),
        )
    }

    pub fn apply_error_named_enum_invalid_prop(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            nodes,
            format!(
                "named enum invalid property for {}::{} prop: {}",
                enum_id, base, prop
            ),
        )
    }

    pub fn apply_error_wrong_enum_base(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            index,
            nodes,
            format!("wrong enum base expected: {} got: {}", enum_id, base),
        )
    }

    pub fn apply_error_wrong_struct_name(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            index,
            nodes,
            format!("wrong struct name expected: {} got: {}", struct_id, got_id),
        )
    }

    pub fn apply_error_wrong_type_for_struct(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            nodes,
            format!(
                "wrong type for struct: {} prop: {} type:{:?}",
                struct_id, nodes[index].id, nodes[index].value
            ),
        )
    }

    pub fn apply_error_wrong_enum_variant(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            nodes,
            format!(
                "wrong enum variant for enum: {} got variant: {}",
                enum_id, variant
            ),
        )
    }

    pub fn apply_error_expected_enum(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            nodes,
            format!(
                "expected enum value type, but got {} {:?}",
                nodes[index].id, nodes[index].value
            ),
        )
    }

    pub fn apply_error_expected_array(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
    ...
    pub fn apply_error_no_matching_field(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            index,
            nodes,
            format!("no matching field: {}", nodes[index].id),
        )
    }

    pub fn apply_error_wrong_type_for_value(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            index,
            nodes,
            format!("wrong type for value: {}", nodes[index].id),
        )
    }

    pub fn apply_error_component_not_found(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
    ...
    pub fn apply_error_wrong_value_type_for_primitive(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            nodes,
            format!(
                "wrong value type. Prop: {} primitive: {} value: {:?}",
                nodes[index].id, prim, nodes[index].value
            ),
        )
    }
    /*
    ...
    pub fn apply_error_animation_missing_state(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            nodes,
            format!(
                "animation missing animator: {} {} {:?}",
                track, state_id, ids
            ),
        )
    }

    pub fn apply_error_wrong_animation_track_used(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            nodes,
            format!(
                "encountered value [{}] with track [{}] whilst animating on track [{}]",
                id, expect, got
            ),
        )
    }

    pub fn apply_error_animate_to_unknown_track(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            index,
            nodes,
            format!("unknown track {} in animate_to state_id {}", id, state_id),
        )
    }

    pub fn apply_error_empty_object(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            index,
            nodes,
            format!("Newed empty ClassName, forgot to call 'use'"),
        )
    }

    pub fn apply_key_frame_cannot_be_interpolated(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            index,
            nodes,
            format!("key frame values cannot be interpolated {:?} {:?}", a, b),
        )
    }

    pub fn apply_animate_missing_apply_block(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            index,
            nodes,
            format!("animate missing apply:{{}} block"),
        )
    }

    pub fn apply_error_cant_find_target(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        if id == crate::live_id!(video) {
            self.apply_error(
                ...
                index,
                nodes,
                format!("Video Player widget is currently only supported on Android"),
            );
            return;
        }
        ...
        self.apply_error(
            ...
            index,
            nodes,
            format!("property: {} target class not found", id),
        )
    }

    pub fn apply_image_type_not_supported(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            index,
            nodes,
            format!("Image type not supported {}", path),
        )
    }

    pub fn apply_image_decoding_failed(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            index,
            nodes,
            format!("Image decoding failed {} {}", path, msg),
        )
    }

    pub fn apply_resource_not_loaded(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            index,
            nodes,
            format!("Resource not loaded {} {}", path, msg),
        )
    }
    ...
    pub fn apply_error_resource_not_found(
        &mut self,
        origin: LiveErrorOrigin,
        index: usize,
        nodes: &[LiveNode],
        ...
        self.apply_error(
            ...
            index,
            nodes,
            format!("resource not found at path {}", path),
        )
    }
    ...
    pub fn apply_error(
        ...
        if let Some(token_id) = &nodes[index].origin.token_id() {
                ...
                .is_some()
            {
                ...
                error!(
                    "Apply error: {} {:?}",
                    live_registry.live_error_to_live_file_error(err),
                    nodes[index].value
                );
            }
        } else {
            error!(
                "Apply without file, at index {} {} origin: {}",
    ...
    pub fn set_dsl_value(&self, crate_id: LiveId, file: LiveId, node: LiveId, value: LiveValue) {
        ...
        let file_id = live_registry
            .module_id_to_file_id(LiveModuleId(crate_id, file))
            .unwrap();
        ...
        if let Some(index) = file
            .original
            .nodes
            .child_by_name(0, LiveProp::instance(node))
        {
                ...
                for node in &file.expanded.nodes {
                    match &node.value {
                        LiveValue::Dependency(dep) => {
                            self.dependencies
                                .insert(dep.as_str().to_string(), CxDependency { data: None });
                        }
                        LiveValue::Font(font) => {
                            for path in &*font.paths {
                                self.dependencies
                                    .insert(path.as_str().to_string(), CxDependency { data: None });
                            }
                        }
                        _ => {}
                    }
    ...
    pub fn register_live_body(&mut self, live_body: LiveBody) {
        ...
        let result = self.live_registry.borrow_mut().register_live_file(
            ...
            TextPos {
                line: live_body.line as u32,
                column: live_body.column as u32,
            },
        );
        //println!("END");
    ...
    */
    ...
    where
        CB: FnOnce(&mut Cx, LiveFileId, usize, &[LiveNode]) -> usize,
    {
        let live_registry_rc = self.live_registry.clone();
        let live_registry = live_registry_rc.borrow();
        ...
        if next_index <= live_ptr.index as usize + 1 {
            self.apply_error_empty_object(
                live_error_origin!(),
                live_ptr.index as usize,
                &doc.nodes,
            );
        }
    }
