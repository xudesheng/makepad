use {
    crate::{
        cx::Cx,
        id_pool::*,
        makepad_live_compiler::{LiveId, LiveType},
...
impl GeometryFingerprint {
    pub fn new(live_type: LiveType) -> Self {
        Self {
            live_type,
            inputs_stored: 0,
            inputs: [0f32; MAX_GEOM_FINGERPRINT],
        }
    }
    pub fn push(&mut self, f: f32) {
        self.inputs[self.inputs_stored] = f;
        self.inputs_stored += 1;
    }
}
