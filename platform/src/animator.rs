use {
    crate::{
        ...
        makepad_live_compiler::{
            LiveFieldKind, LiveIdAsProp, LiveModuleId, LiveNode, LiveNodeOrigin, LiveNodeReader,
            LiveNodeSliceApi, LiveNodeVecApi, LivePropType, LiveRef, LiveType, LiveTypeField,
...
pub trait AnimatorImpl {
    ...
    fn animator_toggle(
        ...
        self.animator_toggle_with_scope(
            cx,
            is_state_1,
            animate,
            state1,
    ...
    fn apply(
        ...
        if let Some(file_id) = apply.from.file_id() {
            self.live_ptr = Some(
                cx.live_registry
                    .borrow()
                    .file_id_index_to_live_ptr(file_id, start_index),
            );
        }
...
impl Animator {
    ...
    pub fn handle_event(&mut self, cx: &mut Cx, event: &Event) -> AnimatorAction {
        if let Event::NextFrame(nf) = event {
            ...
            let mut state_index = state_nodes
                .child_by_name(0, live_id!(state).as_field())
                .unwrap();
            ...
            while state_index < state_nodes.len() {
                ...
                if state_node.is_array() {
                    ...
                    let (play_ended, play_redraw) =
                        Self::update_timeline_value(cx, state_index, state_nodes, nf.time);
                    if !play_ended {
                        ended = false;
                    }
                    if play_redraw {
                    ...
                    // we have to create a timeline ourselves
        ...
        if nodes[index].is_array() {
            ...
            let (ended, time, redraw, track_id) = if let Some(id_index) = node_iter {
                if let LiveValue::Id(track_id) = nodes[id_index].value {
                    ...
                    let track_index = nodes
                        .child_by_path(0, &[live_id!(tracks).as_field(), track_id.as_field()])
                        .unwrap();

                    let time_index = if let Some(time_index) =
                        nodes.child_by_name(track_index, live_id!(time).as_field())
                    {
                        time_index
                    } else {
                        return (true, false);
                    ...
                    let start_time = match &nodes[time_index].value {
                        ...
                        LiveValue::Float64(time) => *time,
                        _ => panic!(),
                    };

                    let play = if let Some(play_index) =
                        nodes.child_by_name(track_index, live_id!(play).as_field())
                    {
                    ...
                    if ended {
                        // mark ended step 1
                        if let Some(index) =
                            nodes.child_by_name(track_index, live_id!(ended).as_field())
                        {
                            nodes[index].value = LiveValue::Int64(cx.event_id as i64);
                        }
                    }

                    let redraw = if let Some(index) =
                        nodes.child_by_name(track_index, live_id!(redraw).as_field())
                    {
            ...
            let default_ease = if let Some(ease_index) = nodes.child_by_path(
                0,
                &[
                    live_id!(tracks).as_field(),
                    track_id.as_field(),
                    live_id!(ease).as_field(),
                ],
            ) {
            ...
            while let Some(node_index) = node_iter {
                if nodes[node_index + 1].is_close() {
                    // at last slot
                    last_child_index = node_index;
                    break;
                }
                let next_kf = if nodes[node_index].is_value_type() {
                    // we hit a bare value node
                    if prev_kf.is_some() {
                        KeyFrame {
                    ...
                    // try to deserialize a keyframe
                    let mut kf =
                        KeyFrame::new_apply(cx, &mut ApplyFrom::New.into(), node_index, nodes);
                    if nodes
                        .child_by_name(node_index, live_id!(ease).as_field())
                        .is_none()
                    {
                    ...
                    if time >= prev_kf.time && time <= next_kf.time {
                        ...
                        let new_val = match a {
                            LiveValue::Int64(va) => match b {
                                ...
                                LiveValue::Float64(vb) => LiveValue::Float64(
                                    ((vb - *va as f64) as f64) * mix + *va as f64,
                                ),
                                _ => LiveValue::None,
                            },
                            LiveValue::Float64(va) => match b {
                                LiveValue::Int64(vb) => LiveValue::Float64(
                                    ((*vb as f64 - va) as f64) * mix + *va as f64,
                                ),
                                LiveValue::Float64(vb) => LiveValue::Float64((vb - va) * mix + *va),
                                _ => LiveValue::None,
                            },
                            LiveValue::Color(va) => match b {
                                LiveValue::Color(vb) => LiveValue::Color(
                                    Vec4::from_lerp(
                                        Vec4::from_u32(*va),
                                        Vec4::from_u32(*vb),
                                        mix as f32,
                            ...
                            LiveValue::Id(_) => match b {
                                LiveValue::Id(vb) => LiveValue::Id(*vb),
                                _ => LiveValue::None,
                            },
                            _ => LiveValue::None,
                        };
                        if let LiveValue::None = &new_val {
                            cx.apply_key_frame_cannot_be_interpolated(
                                live_error_origin!(),
                                index,
                                nodes,
                                a,
    ...
    pub fn is_track_animating(&self, cx: &mut Cx, track_id: &[LiveId; 1]) -> bool {
        if let Some(state) = self.state.as_ref() {
            if let Some(LiveValue::Int64(ended)) = state.child_value_by_path(
                0,
                &[
                    live_id!(tracks).as_field(),
                    track_id[0].as_field(),
                    live_id!(ended).as_field(),
                ],
            ) {
        ...
        if self.need_init() {
            if let Some(live_ptr) = self.live_ptr {
                ...
                if live_registry.generation_valid(live_ptr) {
                    ...
                    if let Some(LiveValue::Id(default_id)) = nodes.child_value_by_path(
                        index,
                        &[
                            check_state_pair[0].as_instance(),
                            live_id!(default).as_field(),
                        ],
                    ) {
            ...
            if let Some(LiveValue::Id(id)) = &state.child_value_by_path(
                0,
                &[
                    live_id!(tracks).as_field(),
                    check_state_pair[0].as_field(),
                    live_id!(state_id).as_field(),
                ],
            ) {
        ...
        if let Some(live_ptr) = self.live_ptr {
            ...
            if live_registry.generation_valid(live_ptr) {
                ...
                if let Some(index) = nodes.child_by_path(
                    index,
                    &[state_id[0].as_instance(), state_id[1].as_instance()],
                ) {
    ...
    pub fn cut_to(
        ...
        state.replace_or_insert_last_node_by_path(
            0,
            &[live_id!(tracks).as_field(), track.as_field()],
            live_object! {
        ...
        let mut reader = if let Some(reader) =
            LiveNodeReader::new(index, nodes).child_by_name(live_id!(apply).as_field())
        {
            reader
        } else {
        ...
        while !reader.is_eot() {
            if reader.is_array() {
                ...
                if let Some(last_value) =
                    Self::last_keyframe_value_from_array(reader.index(), reader.nodes())
                {
                    state.replace_or_insert_first_node_by_path(
                        0,
                        &path,
                        live_array! {
    ...
    pub fn init_as_needed(&mut self, cx: &mut Cx, index: usize, nodes: &[LiveNode]) {
        if self.need_init() {
            ...
            while !nodes[index].is_close() {
                ...
                if let Some(LiveValue::Id(state_id)) =
                    nodes.child_value_by_path(index, &[live_id!(default).as_field()])
                {
    ...
    pub fn animate_to_live(&mut self, cx: &mut Cx, state_pair: &[LiveId; 2]) {
        if let Some(live_ptr) = self.live_ptr {
            ...
            if live_registry.generation_valid(live_ptr) {
                ...
                if let Some(index) = nodes.child_by_path(
                    index,
                    &[state_pair[0].as_instance(), state_pair[1].as_instance()],
                ) {
                    self.animate_to(cx, state_pair, index, nodes)
                } else if !self.ignore_missing {
                    error!(
                        "animate_to_live {}.{} not found",
    ...
    pub fn animate_to(
        ...
        let from_id = if let Some(LiveValue::Id(id)) = state.child_value_by_path(
            0,
            &[
                live_id!(tracks).as_field(),
                track.as_field(),
            ...
            *id
            ...
            cx.apply_error_animate_to_unknown_track(
                live_error_origin!(),
                index,
                nodes,
                track,
        ...
        while !reader.is_eot() {
            if reader.is_array() {
                ...
                let (first_index, last_index) =
                    if let Some(state_child) = state.child_by_path(0, &path) {
                        ...
                        cx.apply_error_animation_missing_state(
                            live_error_origin!(),
                            index,
                            nodes,
                            track,
                ...
                if let LiveValue::Id(check_id) = &state[first_index].value {
                    if *check_id != track {
                        cx.apply_error_wrong_animation_track_used(
                            live_error_origin!(),
                            index,
                            nodes,
                            path.last().unwrap().0,
                ...
                if first_time != 0.0 {
                    // insert first key from the last value
