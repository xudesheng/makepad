...
use {
    crate::{
        ...
        cx::Cx,
        event::{
            Event, KeyEvent, KeyModifiers, MouseButton, MouseDownEvent, MouseMoveEvent,
            MouseUpEvent, ScrollEvent, TextInputEvent, TimerEvent,
        },
    ...
    pub fn get_dispatch(&mut self) -> Vec<Event> {
        ...
        for (id, timer) in self.timers.iter_mut() {
            ...
            if elapsed_time > next_due_time {
                to_be_dispatched.push(Event::Timer(TimerEvent {
                    timer_id: *id,
                    time: Some(time),
                }));
                if timer.repeats {
