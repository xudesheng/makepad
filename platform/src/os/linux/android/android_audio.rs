#![allow(dead_code)]
use {
    self::super::{aaudio_sys::*, android_jni::*},
...
impl AndroidAudioStreamData {
    ...
    unsafe fn verify_channel_count(&mut self, stream: *mut AAudioStream) {
        ...
        if self.actual_channel_count != self.channel_count {
            crate::log!(
                "Android audio device channel count does not match, todo add handling here"
            );
        }
    }

    unsafe fn setup_builder(
        desc: &AndroidAudioDeviceDesc,
    ) -> Result<*mut AAudioStreamBuilder, AndroidAudioError> {
...
impl AndroidAudioOutput {
    unsafe fn new(
        ...
        AAudioStreamBuilder_setDataCallback(
            builder,
            Some(Self::aaudio_stream_data_callback),
            output_stream_ptr as *mut c_void,
        );
        AAudioStreamBuilder_setErrorCallback(
            builder,
            Some(Self::aaudio_stream_error_callback),
            output_stream_ptr as *mut c_void,
        );
    ...
    unsafe extern "C" fn aaudio_stream_data_callback(
        ...
        if let Some(output_fn) = &mut *output_fn {
            data.audio_buffer
                .resize(frame_count as usize, data.channel_count);
            output_fn(
                AudioInfo {
            ...
            let output = std::slice::from_raw_parts_mut(
                ...
                frame_count as usize * data.actual_channel_count,
            );
...
impl AndroidAudioInput {
    unsafe fn new(
        ...
        AAudioStreamBuilder_setDataCallback(
            builder,
            Some(Self::aaudio_stream_data_callback),
            input_stream_ptr as *mut c_void,
        );
        AAudioStreamBuilder_setErrorCallback(
            builder,
            Some(Self::aaudio_stream_error_callback),
    ...
    unsafe extern "C" fn aaudio_stream_data_callback(
        ...
        if let Some(input_fn) = &mut *input_fn {
            ...
            data.audio_buffer
                .resize(frame_count as usize, data.channel_count);
            data.audio_buffer
                .copy_from_interleaved(data.channel_count, &input_data);
            input_fn(
                AudioInfo {
                    device_id: data.device_id,
                    time: None,
...
impl AndroidAudioAccess {
    pub fn new(change_signal: SignalToUI) -> Arc<Mutex<Self>> {
        ...
        Arc::new(Mutex::new(AndroidAudioAccess {
            change_signal,
    ...
    fn java_ret_to_device(
        input: &str,
        device_type: AudioDeviceType,
    ) -> Option<AndroidAudioDeviceDesc> {
    ...
    pub fn use_audio_inputs(&mut self, devices: &[AudioDeviceId]) {
        ...
        for (index, device_desc) in new {
            ...
            match unsafe {
                AndroidAudioInput::new(&device_desc, self.change_signal.clone(), input_cb)
            } {
                Ok(new_input) => {
    ...
    pub fn use_audio_outputs(&mut self, devices: &[AudioDeviceId]) {
        ...
        for (index, device_desc) in new {
            ...
            match unsafe {
                AndroidAudioOutput::new(&device_desc, self.change_signal.clone(), output_cb)
            } {
                Ok(new_output) => {
