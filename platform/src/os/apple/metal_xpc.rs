use {
    crate::{
        //makepad_error_log::*,
        apple_util::nsstring_to_string,
        makepad_objc_sys::{objc_block, objc_block_invoke},
        os::{apple::apple_sys::*, apple_util::str_to_nsstring, cx_stdin::PresentableImageId},
    },
...
fn get_metal_xpc_classes() -> &'static MetalXPCClasses {
    unsafe { &*(METAL_XPC_CLASSES) }
}
fn get_metal_xpc_storage() -> &'static MetalXPCStorage {
    unsafe { &*(METAL_XPC_STORAGE) }
}
...
pub fn xpc_service_proxy() -> RcObjcId {
    unsafe {
        ...
        let proxy: ObjcId =
            msg_send![connection, remoteObjectProxyWithErrorHandler: &error_handler];
        RcObjcId::from_unowned(NonNull::new(proxy).unwrap())
    }
}

pub fn fetch_xpc_service_texture(
    proxy: ObjcId,
    id: PresentableImageId,
    f: impl Fn(RcObjcId) + 'static,
) {
    unsafe {
        ...
        let completion_block =
            hackily_heapify_block2_obj_u64(&completion_block as *const _ as *const _);
...
pub fn define_xpc_service_delegate() -> *const Class {
    ...
    unsafe {
        decl.add_method(
            sel!(listener: shouldAcceptNewConnection:),
            listener as extern "C" fn(&Object, Sel, ObjcId, ObjcId) -> bool,
        );
    }
...
pub fn define_xpc_service_class() -> *const Class {
    extern "C" fn fetch_texture(
        _this: &Object,
        _: Sel,
        presentable_image_id_u64: u64,
        _padding: u64,
        completion: ObjcId,
    ) {
        let storage = get_metal_xpc_storage();
        if let Some(obj) = storage
            .textures_by_presentable_image_id_u64
            .lock()
            .unwrap()
            .remove(&presentable_image_id_u64)
        {
            unsafe {
    ...
    unsafe {
        decl.add_method(
            sel!(fetchTexture: _padding: with:),
            fetch_texture as extern "C" fn(&Object, Sel, u64, u64, ObjcId),
        );
        decl.add_method(
            sel!(storeTexture: obj: _padding: with:),
            store_texture as extern "C" fn(&Object, Sel, u64, ObjcId, u64, ObjcId),
        );
