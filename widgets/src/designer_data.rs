...
impl DesignerData {
    ...
    pub fn update_from_live_registry(&mut self, cx: &mut Cx) {
        ...
        for (file_path, file_id) in live_registry.file_ids() {
            ...
            fn recur_walk_components(
                ...
                parent_children: &mut SmallVec<[LiveId; 4]>,
            ) -> usize {
                while index < nodes.len().saturating_sub(1) {
                    ...
                    } = &nodes[index].value
                    {
                        ...
                        if main_module_lti.live_type == *live_type
                            || wr.map.get(live_type).is_some()
                        {
                            ...
                            // skip the fluff
                            if class == live_id!(Designer)
                                || class == live_id!(PerformanceView)
            ...
            fn path_split<'a>(
                ...
                if let Some((folder, rest)) = name.split_once("/") {
                    ...
                    // add node to pareht
                    if let Some(OutlineNode::Folder { children, .. })
                    | Some(OutlineNode::Virtual { children, .. }) = map.get_mut(&parent_id)
                    {
    ...
    pub fn find_parent(&self, find_node: LiveId) -> Option<LiveId> {
        for (node_id, node) in &self.node_map {
            match node {
                OutlineNode::Component { children, .. }
                | OutlineNode::Virtual { children, .. }
                | OutlineNode::File { children, .. }
    ...
    pub fn _find_component_by_path(&self, path: &[LiveId]) -> Option<LiveId> {
        fn get_node(
            node: LiveId,
            path: &[LiveId],
            map: &HashMap<LiveId, OutlineNode>,
        ) -> Option<LiveId> {
