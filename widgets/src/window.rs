use crate::{
    debug_view::DebugView, desktop_button::*, makepad_derive_widget::*, makepad_draw::*,
    nav_control::NavControl, performance_view::PerformanceView, view::*, widget::*,
};
...
pub struct Window {
    ...
    #[rust(Mat4::nonuniform_scaled_translation(vec3(0.0004,-0.0004,-0.0004),vec3(-0.25,0.25,-0.5)))]
    xr_view_matrix: Mat4,
    #[deref]
    view: View,
...
impl Widget for Window {
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        if let Event::Actions(actions) = event {
            if self
                .desktop_button(id!(windows_buttons.min))
                .clicked(&actions)
            {
                self.window.minimize(cx);
            }
            if self
                .desktop_button(id!(windows_buttons.max))
                .clicked(&actions)
            {
            ...
            if self
                .desktop_button(id!(windows_buttons.close))
                .clicked(&actions)
            {
