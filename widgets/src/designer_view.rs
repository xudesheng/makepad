use crate::{
    designer_data::*, makepad_derive_widget::*, makepad_draw::*, makepad_platform::studio::*,
    turtle_step::*, view::*, widget::*,
};
...
pub struct DesignerView {
    ...
    #[rust]
    view_file: Option<LiveId>,
    #[rust]
    selected_component: Option<LiveId>,
    #[rust]
    selected_subcomponent: Option<SelectedSubcomponent>,
    #[rust]
    containers: ComponentMap<LiveId, ContainerData>,
    #[redraw]
...
impl DesignerView {
    fn draw_container(
        ...
        ptr: LivePtr,
        name: &str,
        to_widget: &mut DesignerDataToWidget,
    ) {
        ...
        if self.reapply {
            cd.component
                .update_from_ptr_with_scope(cx, Some(ptr), &mut scope);
            cd.container
                .update_from_ptr_with_scope(cx, Some(container_ptr), &mut scope);
            // alright here the component is created
        }
        if self.reapply || is_new {
            cd.container.apply_over(
                cx,
                live! {
    ...
    fn sync_zoom_pan(&self, _cx: &mut Cx) {
        Cx::send_studio_message(AppToStudio::DesignerZoomPan(DesignerZoomPan {
            zoom: self.zoom,
            pan_x: self.pan.x,
            pan_y: self.pan.y,
        }));
    ...
    fn update_rect(
        ...
        id: LiveId,
        rect: Rect,
        pos: &mut Vec<DesignerComponentPosition>,
    ) {
...
impl Widget for DesignerView {
            ...
            Hit::FingerHoverOver(fe) => {
                ...
                for (_id, cd) in self.containers.iter() {
                    ...
                    let actions = cx.capture_actions(|cx| {
                        cd.component.handle_event(
                            cx,
                            &Event::DesignerPick(DesignerPickEvent { abs: abs }),
                            &mut Scope::empty(),
                        )
                    });
                        ...
                        if let Some(action) = action.as_widget_action() {
                            match action.cast() {
                                WidgetDesignAction::PickedBody => {
                                    ...
                                    if let Some(id) = data
                                        .find_component_by_widget_ref(component.as_ref().unwrap())
                                    {
                                        cx.widget_action(
                                            uid,
                                            &scope.path,
                                            DesignerViewAction::Selected {
                                                id,
                                                tap_count: 1,
                                                km: fe.modifiers,
                                            },
                                    ...
                                    self.selected_subcomponent = Some(SelectedSubcomponent {
                    ...
                    if fe.modifiers.control && fe.modifiers.alt {
                        ...
                        if fe.modifiers.logo || self.selected_subcomponent.is_none() {
                            ...
                            if let Some(sc) = &self.selected_subcomponent {
                                self.finger_move =
                                    Some(FingerMove::DragSubComponentOrder(sc.clone()));
                            }
                        }
                    }
                }
                                ...
                                if let Some(dir) = reorder {
                                    ...
                                    if !data.pending_revision {
                                        ...
                                        if let Some(comp) = data.find_component_by_widget_ref(c1) {
                                            ...
                                            let next_comp =
                                                data.find_component_by_widget_ref(c2).unwrap();
                                            cx.widget_action(
                                                uid,
                                                &scope.path,
                                                DesignerViewAction::SwapComponents {
                                                    comp,
                                                    next_comp,
                                                },
                                            );
    ...
    fn draw_walk(&mut self, cx: &mut Cx2d, scope: &mut Scope, walk: Walk) -> DrawStep {
                ...
                match data.node_map.get(view_file) {
                    Some(OutlineNode::File { children, .. }) => {
                        for child in children {
                            ...
                            // datastructure
                            if let Some(OutlineNode::Component { ptr, name, .. }) =
                                data.node_map.get(child)
                            {
                                if name == "App=<App>" {
                                    // we need to skip inwards to
                                    if let Some(child) =
                                        data.get_node_by_path(*child, "ui:/main_window=/body=")
                                    {
                                        if let Some(OutlineNode::Component { ptr, name, .. }) =
                                            data.node_map.get(&child)
                                        {
        ...
        self.draw_bg
            .draw_vars
            .set_texture(0, self.color_texture.as_ref().unwrap());
        ...
        cx.set_pass_area_with_origin(&self.pass, self.area, dvec2(0.0, 0.0));
