use {
    crate::{
        fold_button::*, makepad_derive_widget::*, makepad_draw::*, scroll_bars::ScrollBars,
...
pub struct DesignerOutlineTree {
    ...
    #[rust]
    scroll_into_view_id: Option<LiveId>,
    #[rust]
    scroll_into_view_rect: Option<Rect>,

    #[rust]
    tree_nodes: ComponentMap<LiveId, (DesignerOutlineTreeNode, LiveId)>,
...
impl DesignerOutlineTreeNode {
    ...
    pub fn set_is_open(&mut self, cx: &mut Cx, is: bool, animate: Animate) {
        ...
        self.button_open
            .animator_toggle(cx, is, animate, id!(active.on), id!(active.off));
    }
...
impl DesignerOutlineTree {
    ...
    pub fn end(&mut self, cx: &mut Cx2d) {
        ...
        while walk < height_left {
            ...
            self.filler.draw_walk(
                cx,
                Walk::new(
                    Size::fill(),
                    Size::Fixed(self.node_height.min(height_left - walk)),
                ),
            );
        ...
        if let Some(rect) = self.scroll_into_view_rect.take() {
            let rect = rect
                .add_margin(dvec2(0.0, self.node_height * 3.0))
                .translate(self.scroll_bars.get_scroll_pos());
        ...
        if self.should_node_draw(node_id, cx) {
            ...
            if let Some(ptr) = self.templates.get(&template) {
                ...
                tree_node.draw(
                    cx,
                    name,
                    Self::is_even_as_f32(self.count),
                    self.node_height,
                    self.stack.len(),
                    scale,
                    true,
                ...
                OutlineTreeNodeAction::ShouldStartDrag => {
                    if self.dragging_node_id.is_none() {
                        cx.widget_action(
                            uid,
                            &scope.path,
                            OutlineTreeAction::ShouldStartDrag(node_id),
                        );
                    }
                }
            }
