...
impl ImageBuffer {
    ...
    pub fn from_png(data: &[u8]) -> Result<Self, ImageError> {
        ...
        let (width, height) = decoder.get_dimensions().ok_or(ImageError::PngDecode(
    ...
    fn decode_animated_png(decoder: &mut PngDecoder<&[u8]>) -> Result<ImageBuffer, ImageError> {
        let colorspace = decoder.get_colorspace().ok_or(ImageError::PngDecode(
            PngDecodeErrors::GenericStatic("Failed to get animated PNG colorspace"),
        ))?;
        let (width, height) = decoder.get_dimensions().ok_or(ImageError::PngDecode(
            PngDecodeErrors::GenericStatic("Failed to get animated PNG image dimensions"),
        ))?;
        let actl_info =
            decoder
                .actl_info()
...
pub trait ImageCacheImpl {
    ...
    fn load_image_file_by_path(
        ...
        if let Some(ImageCacheEntry::Loaded(texture)) =
            cx.get_global::<ImageCache>().map.get(image_path)
        {
            self.set_texture(Some(texture.clone()), id);
            Ok(())
        } else {
            if let Ok(mut f) = File::open(image_path) {
                let mut data = Vec::new();
                match f.read_to_end(&mut data) {
                    Ok(_len) => self.load_image_file_by_path_and_data(cx, &data, id, image_path),
                    Err(err) => {
                        error!(
                            "load_image_file_by_path: Resource not found {:?} {}",
                            image_path, err
                        );
    ...
    fn load_image_dep_by_path(
        ...
        if let Some(ImageCacheEntry::Loaded(texture)) =
            cx.get_global::<ImageCache>().map.get(p_image_path)
        {
            self.set_texture(Some(texture.clone()), id);
            Ok(())
        } else {
            match cx.take_dependency(image_path) {
                ...
                Err(err) => {
                    error!(
                        "load_image_dep_by_path: Resource not found {} {}",
                        image_path, err
                    );
                    Err(ImageError::PathNotFound(image_path.into()))
                }
