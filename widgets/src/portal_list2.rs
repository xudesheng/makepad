...
pub struct PortalList2 {
    ...
    #[rust]
    draw_lists: ComponentMap<usize, WidgetDrawList>,
...
impl PortalList2 {
    ...
    fn end(&mut self, cx: &mut Cx2d) {
        ...
        if let Some(ListDrawState::Ended {
            min,
            max,
            viewport: _,
        }) = self.draw_state.get()
                    ...
                    match direction {
                        ...
                        DrawDirection::Up => {
                            ...
                            if next_pos < 0.0 || index == 0 {
                                self.draw_state
                                    .set(ListDrawState::Ended { viewport, min, max });
                                return None;
                            } else {
                                self.draw_state.set(ListDrawState::BeginItem {
                                    index: index - 1,
    ...
    pub fn fails_sanity_check_first_id_within_item_range(&self) -> bool {
        !self.tail_range && (self.first_id > self.range_end)
    }
}
...
impl Widget for PortalList2 {
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        let vi = self.vec_index;
        let is_scroll = if let Event::Scroll(_) = event {
            true
        } else {
            false
        };
...
impl PortalList2Ref {
    ...
    pub fn smooth_scroll_to(
        ...
        if target_id < inner.range_start || target_id > inner.range_end {
            return;
        };
        ...
        if let Some(start) = starting_id {
            log!(
                "smooth_scroll_to(): jumping from first ID {} to start ID {}",
                inner.first_id,
                start
            );
    ...
    pub fn smooth_scroll_reached(&self, actions: &Actions) -> bool {
        if let PortalListAction::SmoothScrollReached =
