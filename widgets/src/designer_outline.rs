use crate::{
    designer_data::*, designer_outline_tree::*, makepad_derive_widget::*, makepad_draw::*,
    view::View, widget::*,
};
    ...
    fn draw_walk(&mut self, cx: &mut Cx2d, scope: &mut Scope, _walk: Walk) -> DrawStep {
        ...
        while let Some(next) = self.view.draw(cx, &mut Scope::empty()).step() {
            if let Some(mut file_tree) = file_tree.borrow_mut_if_eq(&next) {
                if let OutlineNode::Virtual { children, .. } =
                    &data.node_map.get(&data.root).as_ref().unwrap()
                {
        ...
        fn recur_nodes(
            cx: &mut Cx2d,
            outline_tree: &mut DesignerOutlineTree,
            ...
            for child in children {
                match map.get(&child).unwrap() {
                    OutlineNode::Folder { name, children } => {
                        if outline_tree
                            .begin_node(cx, *child, &name, live_id!(Folder))
                            .is_ok()
                        {
                    ...
                    OutlineNode::File {
                        ...
                        if children.len() > 0 {
                            if outline_tree
                                .begin_node(cx, *child, &name, live_id!(File))
                                .is_ok()
                            {
                                recur_nodes(cx, outline_tree, map, children);
                                outline_tree.end_node();
                    ...
                    OutlineNode::Component { children, name, .. } => {
                        if children.len() > 0 {
                            if outline_tree
                                .begin_node(cx, *child, &name, live_id!(Layout))
                                .is_ok()
                            {
                                recur_nodes(cx, outline_tree, map, children);
                                outline_tree.end_node();
