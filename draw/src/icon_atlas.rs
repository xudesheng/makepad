pub use {
    crate::{
        ...
        makepad_platform::*,
        makepad_vector::geometry::{AffineTransformation, Point, Transform, Vector},
        makepad_vector::internal_iter::*,
        makepad_vector::path::{PathCommand, PathIterator},
        makepad_vector::trapezoidator::Trapezoidator,
        shader::draw_trapezoid::DrawTrapezoidVector,
        turtle::{Layout, Walk},
    },
    makepad_html::*,
    std::{cell::RefCell, collections::HashMap, fs::File, io::prelude::*, rc::Rc, sync::Arc},
};
...
impl CxIconAtlas {
    ...
    pub fn parse_and_cache_path(
        &mut self,
        path_hash: CxIconPathHash,
        path: &[u8],
    ) -> Option<(CxIconPathHash, Rect)> {
        match parse_svg_path(path) {
            Ok(path) => {
                ...
                if let Some(foundpath) = self.paths.get_mut(&path_hash) {
                    foundpath.push(CxIconPathCommands { bounds, path })
                } else {
                    self.paths
                        .insert(path_hash, vec![CxIconPathCommands { bounds, path }]);
                }
    ...
    pub fn get_icon_bounds(
        ...
        if svg_dep.len() != 0 {
            ...
            self.svg_deps
                .insert(svg_dep.as_str().to_string(), path_hash);
                ...
                Ok(data) => {
                    ...
                    while !node.done() {
                        match node.open_tag_lc() {
                            ...
                            some_id!(path) => {
                                self.parse_and_cache_path(
                                    path_hash,
                                    node.find_attr_lc(live_id!(d)).unwrap().as_bytes(),
                                );
                            }
    ...
    pub fn get_icon_slot(&mut self, args: CxIconArgs, path_hash: CxIconPathHash) -> CxIconSlot {
        ...
        let (slot, pos) = self
            .alloc
            .alloc_icon_slot(args.size.x as f64, args.size.y as f64);
        self.entries.insert(
            entry_hash,
            CxIconEntry {
                path_hash,
    ...
    pub fn alloc_icon_slot(&mut self, w: f64, h: f64) -> (CxIconSlot, DVec2) {
        ...
        (
            CxIconSlot {
                ...
                t2: dvec2(
                    tx1 + (w / self.texture_size.x),
                    ty1 + (h / self.texture_size.y),
                )
                .into(),
            },
            dvec2(px, py).into(),
...
impl DrawTrapezoidVector {
    ...
    fn draw_vector(
        &mut self,
        entry: &CxIconEntry,
        path: &CxIconPathCommands,
        many: &mut ManyInstances,
    ) {
        let trapezoids = {
                        ...
                        let cmd = cmd.transform(
                            &AffineTransformation::identity()
                                .translate(Vector::new(
                                    entry.args.translate.x,
                                    entry.args.translate.y,
                                ))
                                .uniform_scale(entry.args.scale)
                                .translate(Vector::new(
                                    entry.pos.x + entry.args.subpixel.x,
                                    entry.pos.y + entry.args.subpixel.y,
                                )),
                        );
                        cmd
                    }
...
impl CxDrawIconAtlas {
    pub fn new(cx: &mut Cx) -> Self {
        let atlas_texture = Texture::new_with_format(
            cx,
...
impl<'a> CxDraw<'a> {
    ...
    pub fn draw_icon_atlas(&mut self) {
        ...
        if atlas.alloc.todo.len() > 0 {
            ...
            draw_atlas
                .atlas_pass
                .add_color_texture(self.cx, &draw_atlas.atlas_texture, clear);
            ...
            if let Some(mut many) = self.begin_many_instances(&draw_atlas.draw_trapezoid.draw_vars)
            {
                for todo in atlas_todo {
                    ...
                    for i in 0..path.len() {
                        draw_atlas
                            .draw_trapezoid
                            .draw_vector(entry, &path[i], &mut many);
                    }
                }
...
pub fn parse_svg_path(path: &[u8]) -> Result<Vec<PathCommand>, String> {
    ...
    impl ParseState {
        ...
        fn finalize_cmd(&mut self) -> Result<(), String> {
            ...
            if self.expect_nums != self.num_count {
                return Err(format!(
                    "SVG Path command {:?} expected {} points, got {}",
                    self.cmd, self.expect_nums, self.num_count
                ));
            }
            match self.cmd {
                Cmd::Unknown => (),
    ...
    for i in 0..path.len() {
        match path[i] {
            ...
            b'.' => state.add_dot()?,
            b',' | b' ' | b'\r' | b'\n' | b'\t' => state.whitespace()?,
            x => return Err(format!("Unexpected character {} - {}", x, x as char)),
        }
    }
    state.finalize_cmd()?;
