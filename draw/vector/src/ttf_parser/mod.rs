use crate::font::{Glyph, HorizontalMetrics, TTFFont};
...
impl TTFFont {
    pub fn get_glyph_by_id(&mut self, face: &Face<'_>, id: usize) -> Result<&Glyph> {
        ...
        if glyph_slot.is_none() {
            ...
            if self.cached_svg_images[id.0 as usize].is_none() {
                match face.glyph_svg_image(id) {
                    Some(svg_image) => {
                        ...
                        let tree =
                            Rc::new(Tree::from_data(&svg_image.data, &opt).map_err(|_| Error)?);
            ...
            let bounds = face
                .outline_glyph(id, &mut outline_builder)
                .map(
                    |ttf_parser::Rect {
                         x_min,
                         y_min,
            ...
            *glyph_slot = Some(Box::new(Glyph {
                ...
                svg_image: self.cached_svg_images[id.0 as usize]
                    .as_ref()
                    .unwrap()
                    .clone(),
            }));
